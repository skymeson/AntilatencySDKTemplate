...
namespace AntilatencyInterfaceContract {
        ...
        public class IUnsafeWrapper : IUnsafe {
            ...
            protected void HandleExceptionCode(ExceptionCode code) {
                if (code != ExceptionCode.Ok) {
                    var exceptionData = QueryInterface<AntilatencyInterfaceContract.IExceptionData>();
                    throw new AntilatencyInterfaceContract.Exception(code, exceptionData.getMessage());
                }
            }
        }


        public class IUnsafeRemap : IDisposable {
...
#pragma warning restore 0649
            ...
            protected void AllocateNativeInterface(System.IntPtr vmt, System.IntPtr context) {
                ...
                var data = new Data() { Vmt = vmt, Context = context };
            ...
            protected static ExceptionCode handleRemapException(System.Exception ex, IntPtr objPtr) {
                var obj = AntilatencyInterfaceContract.Details.IInterfaceRemap.GetContext(objPtr) as InterfacedObject;
                var data = obj.QueryInterface<AntilatencyInterfaceContract.IExceptionData>();

                data.setMessage(ex.Message);

                if (ex is AntilatencyInterfaceContract.Exception) {
                    return (ex as AntilatencyInterfaceContract.Exception).code;
...
#pragma warning restore 0649
    ...
    public interface IExceptionData : AntilatencyInterfaceContract.IUnsafe {
        string getMessage();
        void setMessage(string message);
    }
    namespace Details {
        public class IExceptionDataWrapper : AntilatencyInterfaceContract.Details.IUnsafeWrapper, IExceptionData {
            ...
            public void setMessage(string message) {
                var valueMarshaler = AntilatencyInterfaceContract.Details.ArrayInMarshaler.create(message);
        ...
        public class IExceptionDataRemap : AntilatencyInterfaceContract.Details.IUnsafeRemap {
            public new struct VMT {
                public delegate AntilatencyInterfaceContract.ExceptionCode getMessageDelegate(System.IntPtr _this, AntilatencyInterfaceContract.Details.ArrayOutMarshaler.Intermediate message);
                public delegate AntilatencyInterfaceContract.ExceptionCode setMessageDelegate(System.IntPtr _this, AntilatencyInterfaceContract.Details.ArrayInMarshaler.Intermediate message);
#pragma warning disable 0649
...
#pragma warning restore 0649
            ...
            protected static new void AppendVmt(System.Collections.Generic.List<object> buffer) {
                AntilatencyInterfaceContract.Details.IUnsafeRemap.AppendVmt(buffer);
                var vmt = new VMT();
                vmt.getMessage = (System.IntPtr _this, AntilatencyInterfaceContract.Details.ArrayOutMarshaler.Intermediate message) => {
                    try {
                ...
                vmt.setMessage = (System.IntPtr _this, AntilatencyInterfaceContract.Details.ArrayInMarshaler.Intermediate message) => {
                    try {
                        var obj = GetContext(_this) as IExceptionData;
    ...
    public abstract class InterfacedObject : IInterface, IExceptionData {
        ...
        public InterfacedObject() {
            ...
            foreach(var i in interfaces) {
                var remapType = AntilatencyInterfaceContract.Details.Utils.GetRemapType(i);
                if(remapType != null) {
...
#pragma warning restore 0649
            ...
            protected static new void AppendVmt(List<object> buffer) {
                ...
                vmt.unloadLibrary = (IntPtr _this) => {
                    var obj = AntilatencyInterfaceContract.Details.IInterfaceRemap.GetContext(_this) as ILibraryUnloader;
                    obj.unloadLibrary();
                    return ExceptionCode.Ok;
                };
                buffer.Add(vmt);
    ...
    namespace Details {
                ...
                public T[] toArray<T>() where T : struct {
                    ...
                    if(size != 0) {
                        for (uint i = 0; i < size; ++i) {
                            result[i] = (T)Marshal.PtrToStructure(readPtr, typeof(byte));
                            readPtr = new IntPtr(readPtr.ToInt64() + Marshal.SizeOf(typeof(byte)));
                        }
                    }
                    return result;
                }
        ...
        public class ArrayOutMarshaler {
        ...
        public class ArrayOutMarshalerImpl<T> : ArrayOutMarshaler {
            ...
            public static setArraySizeDelegate setArraySize = (IntPtr context, uint newSize) => {
                var accessorHandle = GCHandle.FromIntPtr(context);
                var accessor = (ArrayOutMarshalerImpl<T>)accessorHandle.Target;

                Array.Resize<T>(ref accessor._buffer, (int)newSize);

                if(newSize == 0) {
                    return IntPtr.Zero;
                }

                accessor._arrayHandle = GCHandle.Alloc(accessor._buffer, GCHandleType.Pinned);
                return accessor._arrayHandle.AddrOfPinnedObject();
            };
